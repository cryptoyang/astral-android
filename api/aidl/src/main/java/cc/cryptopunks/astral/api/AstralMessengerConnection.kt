...
class AstralMessengerConnection(
    ...
    override fun onServiceConnected(name: ComponentName, binder: IBinder) {
        ...
        val network = binder as? Network ?: Network.Stub.asInterface(binder)
//        val network = Network.Stub.asInterface(binder)
        job = scope.launch(dispatcher) { onBind(network) }
    }
    ...
    private companion object {
        val intent: Intent
            get() = Intent().apply {
                ...
                putExtra("sender", "cc.cryptopunks.astral.service")
            }
    }
}
