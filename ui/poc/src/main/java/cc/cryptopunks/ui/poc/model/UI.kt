...
typealias UIResolveView = UI.State.() -> UIView
...
fun eventHandler(
    ...
    createRequest: UICreateRequest = uiCreateRequest,
...
fun generateMessages(
    ...
    when (event) {
        ...
        UI.Event.Action -> when {
            isReady -> listOf(
                UI.Element.Stack + append(nextView)
            )
...
fun UI.State.append(view: UIResolveView): List<UIView> =
    stack + view()

fun UI.State.isRequiredParam(text: String): Boolean =
